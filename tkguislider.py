...
sig_out = dev.add_signal(mpr.Direction.OUTGOING, "output",
                         1, mpr.Type.FLOAT, None, 0, 100)
...
def do_poll():
    ...
    if dev.ready and not name_known:
        name.set('Device name: %s, listening on port %s' %
                 (dev[mpr.Property.NAME], dev[mpr.Property.PORT]))
        name_known = True
    ui.after(5, do_poll)
